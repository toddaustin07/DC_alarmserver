...
GUI displays are implemented with GTK3+.  Currently for zone device apps, gui windows are optional, but not option for the panel app.  If it turns out there is little interest in the GUIs, I may remove that code for the sake of further efficiency, since it does increase the amount of memory required for each app whether you use the GUI option or not.
