...
This step can be eliminated if the Developer Workspace allowed using custom capabilities outside your own account.  At this time it doesn't, so I am continuing to look for ways to simplify this task.  I'm looking and writing a script that will automate much of this using the API, so watch for updates.
...
You will use the onboarding helper script (wip) and SmartThings mobile app to connect with each of the running zone & panel device apps on your Raspberry Pi via wireless and complete onboarding. 
