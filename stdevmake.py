...
def showhelp():
    ...
    Everthing starts with an existing device configuration definition JSON file, which can be obtained in either of two ways:\n\
         1.The original developer provides the json file to you\n\
         ...
         2.The original developer provides you the Presentation ID and Manufacturer Name and you retrieve the device config using this tool\n\
    \n\
    Use the menu bar 'File->Open local device config json file' for scenario #1 above, or 'File->Retrieve device config from SmartThings'\n\
    for scenario #2 above\n\
    \n\
    Once the device config is loaded, the custom capabilities it contains will be displayed.  Be sure they are all selected and press the\n\
    'Retrieve' buttton to download the custom capability definition JSON files (including presentation JSON).\n\
    \n\
    Once the capabilities are downloaded, you can press the 'CLONE ALL' button to clone the device config and custom capabilities\n\
    to files onto your local disk.  Lastly, click the 'SUBMIT ALL' button and the capabilities will be submitted to SmartThings to be\n\
    created under your own namespace; plus a new device config Presentation ID (vid) will be generated by SmartThings. All JSON returned\n\
    by SmartThings will be saved locally.\n\
    \n\
    The returned device config JSON file can now be uploaded to the Developer Workspace to complete your device profile UI Display\n\
    ...
    The tool also allows for more granular control of working with device configs and custom capabilities:\n\
        ...
        - Instead of using the blue highlighted 'CLONE ALL' and 'SUBMIT ALL' buttons, individual buttons are provided for more\n\
